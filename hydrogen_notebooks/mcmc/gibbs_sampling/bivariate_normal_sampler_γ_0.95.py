...
hmc.cumulative_mean(title, q[:,0], time, 0.0, [-2.1, 1.0], f"{file_prefix}-cummulative-mean-1")

# %%

title = f"Gibbs Bivariate Normal " + r"$q_2$" + f": γ={γ}, nsample={nsample}"
time = range(0, len(q[:,1]))
hmc.cumulative_mean(title, q[:,1], time, 0.0, [-2.1, 1.0], f"{file_prefix}-cummulative-mean-2")
